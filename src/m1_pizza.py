...
def run_test_generate_points_on_circle():
    ...
    expected = [rg.Point(300.0, 100.0),
                rg.Point(250.0, 150.0),
                rg.Point(200.0, 100.0),
                rg.Point(250.0, 50.0)]
    circle = rg.Circle(rg.Point(250, 100), 50)
...
def run_test_draw_points_on_circle():
    ...
    title = 'DRAW_POINTS_ON_CIRCLE, test 2:  3 red dots.'
    window = rg.RoseWindow(400, 400, title)
...
def run_test_pizza():
    ...
    title = 'PIZZA test 4:  12 slices, thin (thickness=3)  lines.'
    window = rg.RoseWindow(400, 400, title)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(number_of_slices):
        line = rg.Line(points[k], points[k + number_of_slices])
...
def run_test_polygon():
    ...
    title = 'POLYGON test 3:  8 segments with thin green lines'
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(200, 200), 100)
    circle.outline_thickness = 3
    polygon(window, circle, 8, 'green', 3)
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4:  12 red lines on black circle, ' \
            'hops = 5.'
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(200, 150), 100)
    circle.fill_color = 'black'
    fancy_polygon(window, circle, 12, 5, 'red', 3)
    window.close_on_mouse_click()
